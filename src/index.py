...
img = Image.open('test01.jpg') # Can be many different formats.
imgPixel = img.load()
...
def visit(x, y, id, kc):
    ...
    for u in range(xL, xR + 1):
        for v in range(yL, yR + 1):
            ...
            if gray_level < 75 and visited[u][v] == 0:
...
for y in range(0, img.height):
    for x in range(0, img.width):
        ...
        color2 = imgPixel[x, y]
        gray_level = 0.299 * color2[0] + 0.587 * color2[1] + 0.114 * color2[2]
        if gray_level < 75 and visited[x][y] == 0:
            #print(x, y)
            x1 = 1000
            x2 = 0
            y1 = 1000
